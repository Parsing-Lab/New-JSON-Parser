...
namespace wiz {
	...
	std::pair<bool, int> Reserve2_3(std::ifstream& inFile, VECTOR<Token2>* aq, const int num, bool* isFirst, const wiz::LoadDataOption& option, int thr_num, char*& _buffer)
	{
		...
		log_result = log_result + (std::chrono::steady_clock::now() - a).count();
					...
					if (bsPair.first) {
						if (bsPair.second.len == 1 && -1 != Equal(option.Assignment, bsPair.second.str[0])) {
							...
							if (i <= last_idx) {
								...
								nestedUT[braceNum]->AddItem("", move(val));
								val = "";
	...
	inline int FindRight(VECTOR<Token2>& strVec, int start, int last, const wiz::LoadDataOption& option)
	{
	...
	//	strVec.emplace_back(left, 1, false);
		...
		while (true) {
...

			{
				...
				//if (first) 
				{
					...
					thr[0] = std::thread(__LoadData3, &strVec, 0, idx, &__global[0], &option, 0, 0, &next[0]);
				}
				//else {
				//	int idx = pivots.empty() ? last_idx : pivots[0];
				//	thr[0] = std::thread(__LoadData3, &strVec, 0, idx, &__global[0], &option, 0, 0, &next[0]);
				...
				for (int i = 1; i < pivots.size(); ++i) {
					thr[i] = std::thread(__LoadData3, &strVec, pivots[i - 1] + 1, pivots[i], &__global[i], &option, 0, 0, &next[i]);
				}
		...
		global = std::move(_global); // *(_global.GetUserTypeList(0)->GetUserTypeList(0)));

		std::cout << "lexing " << (double)std::chrono::steady_clock::duration(log_result).count() 
									* std::chrono::steady_clock::period().num / std::chrono::steady_clock::period().den << "s" 
			<< std::endl;
		log_result = 0;
		std::cout << "parsing " << (double)std::chrono::steady_clock::duration(sum).count()
